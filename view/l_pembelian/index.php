...
<ol class="breadcrumb">
<li><a href="#"><i class="fa fa-bar-chart"></i> Laporan Pembelian</a></li>
</ol>
</section>
        ...
        <div class="box box-primary">
            ...
            <div class="box-body row">
              <form action="#" method="GET">
                ...
                <div class="col-md-3 col-xs-12">
                  <div class="form-group">
                    <label for="pelanggan">ID Transaksi</label>
                    <select class="form-control select2" style="width: 100%;" id="id_customer" name="pelanggan">
                      ...
                      <?php
                      $sql = $conn->prepare("SELECT * FROM t_pembelian GROUP BY id_trx");
                      $sql->execute();
        ...
        <div class="box box-primary">
            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                <?php
                   $count = 1;
                   if($_GET['pelanggan'] == ""){
                    $sql = $conn->prepare("SELECT * FROM `t_pembelian` WHERE DATE(`created_at`) BETWEEN '".date("Y-m-d",strtotime($_GET['tanggal']))."' AND '".date("Y-m-d",strtotime($_GET['sd']))."' ORDER BY id DESC");
                    $sql->execute();
                    while($data=$sql->fetch()) {
                ?>
                ...
                ?>
                Total Pembelian = <?= "Rp. ".number_format($totals,0).",-" ?>
                <?php
                }else if($_GET['pelanggan'] != ""){
                  $sql = $conn->prepare("SELECT * FROM `t_pembelian` WHERE DATE(`created_at`) BETWEEN '".date("Y-m-d",strtotime($_GET['tanggal']))."' AND '".date("Y-m-d",strtotime($_GET['sd']))."' AND id_trx = '".$_GET['pelanggan']."' ORDER BY id DESC");
                  $sql->execute();
                  while($data=$sql->fetch()) {
                ?>
