<?php
	...
	if($op == "tambah"){
        ...
        try {
          $sql = "INSERT INTO m_supplier SET
          nama = :nama,
          des = :des"
          ;
        ...
        if($stmt){		
			echo "<script>alert('Berhasil Tambah'); document.location.href=('../view/m_supplier')</script>";
		}else{
			echo "<script>alert('Gagal Tambah'); document.location.href=('../view/m_supplier')</script>";
		}
		...
		$sql = "DELETE FROM m_supplier WHERE id = :id";
		$stmt = $conn->prepare($sql);
		$stmt->bindParam(':id', $id);
		$stmt->execute();
		
		if($stmt){		
			echo "<script>alert('Berhasil Menghapus'); document.location.href=('../view/m_supplier/')</script>";
		}else{
			echo "<script>alert('Gagal Menghapus'); document.location.href=('../view/m_supplier/')</script>";
		}
