<?php
	...
	if($op == "tambah"){
		...
		if ($cekqty > $qty){
			echo "<script>alert('QTY tidak cukup'); document.location.href=('../view/t_penjualan')</script>";
		}elseif($cekqty <= $qty){
			$nama = $row['nama'];
			$sisaqty = $qty - $cekqty;
			$satuan = $row['satuan'];
			...
			if($stmt){		
				echo "<script>alert('Berhasil Tambah'); document.location.href=('../view/t_penjualan')</script>";
			}else{
				echo "<script>alert('Gagal Tambah'); document.location.href=('../view/t_penjualan')</script>";
			}
		}
	}if($op == "selesai"){
        ...
        if($stmt){		
			echo "<script>alert('Selesai Transaksi'); document.location.href=('../view/t_penjualan/')</script>";
		}else{
			echo "<script>alert('Gagal Tambah'); document.location.href=('../view/t_penjualan')</script>";
		}
		...
		$sql = "UPDATE m_transaksi SET 
		stat = 2 WHERE id = $id";
		$stmt = $conn->prepare($sql);
		$stmt->execute();
		
		if($stmt){		
			echo "<script>alert('Selesai Proses Pengiriman'); document.location.href=('../view/m_transaksi')</script>";
		}else{
			echo "<script>alert('Gagal Proses'); document.location.href=('../view/m_transaksi')</script>";
		}
		...
		if($stmt){		
			echo "<script>alert('Berhasil Menghapus'); document.location.href=('../view/t_penjualan/')</script>";
		}else{
			echo "<script>alert('Gagal Menghapus'); document.location.href=('../view/t_penjualan/')</script>";
		}
		...
		$sql = "DELETE FROM t_penjualan WHERE id_trx = :id_trx";
		$stmt = $conn->prepare($sql);
		$stmt->bindParam(':id_trx', $id_trx);
		$stmt->execute();

		$sql = "DELETE FROM m_kirim WHERE id_trx = :id_trx";
		$stmt = $conn->prepare($sql);
		$stmt->bindParam(':id_trx', $id_trx);
		$stmt->execute();
		...
		if($stmt){		
			echo "<script>alert('Berhasil Menghapus'); document.location.href=('../view/m_transaksi/')</script>";
		}else{
			echo "<script>alert('Gagal Menghapus'); document.location.href=('../view/m_transaksi/')</script>";
		}
