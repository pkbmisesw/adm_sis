<?php
	...
	if($op == "tambah"){
        ...
        try {
          ...
          $stmt = $conn->prepare($sql);
          $stmt->bindParam(':bpr_rate', $bpr_rate);
          $stmt->bindParam(':tabungan', $tabungan);
		  $stmt->bindParam(':simpanan', $simpanan);
          $stmt->bindParam(':depo1', $depo1);
          $stmt->bindParam(':depo3', $depo3);
		  $stmt->bindParam(':depo12', $depo12);
        ...
        if($stmt){		
			echo "<script>alert('Data telah dirubah'); document.location.href=('../view/sukubunga')</script>";
		}else{
			echo "<script>alert('Gagal Data telah dirubah'); document.location.href=('../view/sukubunga')</script>";
		}
