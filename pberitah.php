...
<?php	
	...
	$sql = 'SELECT * FROM m_post WHERE stat = 1 AND nama LIKE :keyword ORDER BY id DESC ';
...
<table class='tbl-qa' >
  <thead>
	<tr>
	  ...
	  <th class='table-header' width='30%'>Gambar</th>
	  <th class='table-header' width='30%'>Judul</th>
	  <th class='table-header' width='40%'>Descripsi</th>
	</tr>
  </thead>
  <tbody id='table-body'>
	<?php
	...
	?>
	  <tr class='table-row'>
	    <td><a href="post?p=<?php echo $row['id'];?>"><img src="images/<?php echo $row['gambar']; ?>" width="100%" height="100%"></a></td>
		<td>
		    <span class="description">Penulis : <?php echo $row['penulis'];?> - <?php echo $row['tgl'];?> - Dilihat : <?php echo $row['view'];?> Kali</span>
		    <a style="color:black;" href="post?p=<?php echo $row['id'];?>"><b><?php echo $row['nama']; ?></b></a>
